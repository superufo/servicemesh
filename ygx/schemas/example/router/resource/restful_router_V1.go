...
func (r *RestFulRouterA) GetPayments(context *restful.Context) {
	...
	val := archaius.GetString("cse.protocols.rest.listenAddress", "")
	//context.Write([]byte(fmt.Sprintf("Ip  %s loadbalance was called", val)))
...
func (r *RestFulRouterA) Info(context *restful.Context) {
	...
	context.WriteJSON(versionInfo, "application/json", "testV1")
}
